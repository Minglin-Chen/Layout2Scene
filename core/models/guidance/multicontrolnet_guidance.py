...
class MultiControlNetGuidance(BaseObject):
    ...
    class Config(BaseObject.Config):
        ...
        controlnet_pretrained_model_name_or_path: Optional[list] = \
            field(default_factory=lambda: [\
                "lllyasviel/control_v11p_sd15_seg",
                "lllyasviel/control_v11p_sd15_normalbae", 
                "lllyasviel/control_v11p_sd15_depth"])
        controlnet_conditioning_scale: Optional[list] = \
            field(default_factory=lambda: [1., 1., 1.])
        controlnet_guess_mode: bool = False

        condition_keys: Optional[list] = \
            field(default_factory=lambda: ["comp_semantic", "comp_normal", "comp_depth"])
    ...
    def forward_controlnet(
        ...
        condition: List[Float[Tensor, "..."]],
        conditioning_scale: List[float] = [1.0, 1.0, 1.0],
        ...
        if guess_mode:
            ...
            condition = [c.chunk(2)[0] for c in condition]
        else:
        ...
        down_block_res_samples, mid_block_res_sample = self.controlnet(
            ...
            controlnet_cond=[c.to(self.weights_dtype) for c in condition],
    ...
    def compute_grad_sds(
        ...
        condition: Optional[List[Float[Tensor, "B 3 512 512"]]],
            ...
            with torch.no_grad():
                ...
                if condition is not None:
                    down_block_res_samples, mid_block_res_sample = self.forward_controlnet(
                        ...
                        condition=[torch.cat([c] * 4) for c in condition],
            ...
            with torch.no_grad():
                ...
                if condition is not None:
                    down_block_res_samples, mid_block_res_sample = self.forward_controlnet(
                        ...
                        torch.cat([t] * 2),
                        encoder_hidden_states=text_embeddings,
                        condition=[torch.cat([c] * 2) for c in condition],
                        conditioning_scale=self.cfg.controlnet_conditioning_scale,
                        guess_mode=self.cfg.controlnet_guess_mode
                    )
                else:
    ...
    def __call__(
        ...
        prompt_utils: PromptProcessorOutput,
        condition: Optional[List[Float[Tensor, "B HH WW 3"]]],
        elevation: Float[Tensor, "B"],
        azimuth: Float[Tensor, "B"],
        camera_distances: Float[Tensor, "B"],
        ...
        **kwargs,
        ...
        if condition is not None:
            ...
            for c in condition:
                c = c.permute(0, 3, 1, 2)
                c = F.interpolate(c, (512, 512), mode="bilinear", align_corners=False)
                c = c.repeat(1,3,1,1) if c.shape[1] == 1 else c
    ...
    def guidance_eval(
        ...
        for b, i in enumerate(idxs):
            ...
            cond = [c[b : b + 1] for c in condition] if isinstance(condition, list) else condition[b : b + 1]
            text_emb = (
                text_embeddings[
            ...
            for t in tqdm(self.scheduler.timesteps[i + 1 :], leave=False):
                ...
                noise_pred = self.get_noise_pred(
                    latents, t, text_emb, cond, use_perp_neg, neg_guid
                )
                # get prev latent
