...
@threestudio.register("nddiffusion-guidance")
class NDDiffusionGuidance(BaseObject):
    @dataclass
    class Config(BaseObject.Config):
        pretrained_config: str = "nddiffusion/configs/nd-1.5-inference.yaml"
        pretrained_model_name_or_path: str = "nddiffusion/ckpt/nd-laion_ema.ckpt"
    ...
    def configure(self) -> None:
        threestudio.info(f"Loading Normal Depth Diffusion ...")
    ...
    def compute_grad_sds(
        ...
        if prompt_utils.use_perp_neg:
            ...
            with torch.no_grad():
                ...
                t_in = torch.cat([t] * 4)
                with autocast('cuda'):
                    noise_pred = self.model.apply_model(x_in, t_in, text_embeddings) # (4B, 4, 64, 64)
            ...
            with torch.no_grad():
                ...
                t_in = torch.cat([t] * 2)
                with autocast('cuda'):
                    noise_pred = self.model.apply_model(x_in, t_in, text_embeddings)
    ...
    def get_noise_pred(
        ...
        if use_perp_neg:
            # pred noise
            x_in = torch.cat([latents_noisy] * 4, dim=0)
            t_in = torch.cat([t.reshape(1)] * 4).to(self.device)
            with autocast('cuda'):
                noise_pred = self.model.apply_model(x_in, t_in, text_embeddings)
                # (4B, 4, 64, 64)
