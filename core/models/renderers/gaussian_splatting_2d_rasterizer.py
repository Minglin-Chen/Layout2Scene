...
class GaussianSplatting2DRasterizer(Rasterizer):
    ...
    def __splatting_one(
        ...
        rendered_distort = allmap[6:7]

        # psedo surface attributes
        # surf depth is either median or expected by setting depth_ratio to 1 or 0
        # for bounded scene, use median depth, i.e., depth_ratio = 1;
        # for unbounded scene, use expected depth, i.e., depth_ration = 0, to reduce disk anliasing.
        # rendered_depth = render_depth_expected * (1-self.cfg.depth_ratio) + (self.cfg.depth_ratio) * render_depth_median
        rendered_depth = render_depth_median
        ...
        rendered_normal             = rendered_normal.unsqueeze(0).permute(0,2,3,1)
        rendered_distort            = rendered_distort.unsqueeze(0).permute(0,2,3,1)
        ...
        comp_normal     = surf_normal # options: rendered_normal, surf_normal
        comp_distort    = rendered_distort
        radii           = radii / float(max(width, height))

        return \
            comp_rgb, opacity, comp_semantic, comp_depth, comp_normal, comp_distort, means2D, radii
    ...
    def __splatting_batch_vanilla(
        ...
        for _c2w, _fovy in zip(c2w, fovy):
            ...
            rgb, opacity, semantic, depth, normal, surf_normal, \
                depth_distortion, viewspace_points, visibility_filter, radii = \
                    self.__splatting_one(_c2w, _fovx, _fovy, height, width, local, local_index)
    ...
    def __splatting_batch_gsplat(
        ...
        # rendering
        rendered_results, rendered_alpha, meta = gsplat.rasterization_2dgs(
            ...
            distloss=True,
            depth_mode='expected', # options: expected, median
        )
