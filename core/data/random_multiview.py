...
class RandomMultiviewCameraIterableDataset(RandomCameraIterableDataset):
    def __init__(self, cfg: any) -> None:
        super().__init__(cfg)
        self.cfg: RandomMultiviewCameraDataModuleConfig = cfg
    ...
    def collate(self, batch) -> Dict[str, Any]:
        ...
        up: Float[Tensor, "B 3"] = torch.as_tensor([0, 0, 1], dtype=torch.float32)[
            None, :
        ].repeat(self.batch_size * self.cfg.n_view, 1)
        ...
        if self.cfg.light_sample_strategy == "dreamfusion":
            ...
            light_direction: Float[Tensor, "B 3"] = F.normalize(
                camera_positions
                + torch.randn(self.batch_size, 3).repeat_interleave(
        ...
        directions: Float[Tensor, "B H W 3"] = self.directions_unit_focal[
            None, :, :, :
        ].repeat(self.batch_size * self.cfg.n_view, 1, 1, 1)
        directions[:, :, :, :2] = (
