...
def log_validation(
    ...
    for validation_prompt, validation_image in zip(validation_prompts, validation_images):
        ...
        for _ in range(args.num_validation_images):
            ...
            depth = np.concatenate([depth]*3, axis=-1)
            normals.append(Image.fromarray(normal))
            depths.append(Image.fromarray(depth))

        image_logs.append(
            {"validation_image": validation_image, "normals": normals, "depths": depths, "validation_prompt": validation_prompt}
        )
        ...
        if tracker.name == "tensorboard":
            for log in image_logs:
                ...
                tracker.writer.add_images(
                    "Conditions", 
                    np.stack([np.asarray(validation_image)]), 
                    step, 
                    dataformats="NHWC")

                formatted_images = []

                for normal, depth in zip(normals, depths):
                    formatted_images.append(np.asarray(normal))
                    formatted_images.append(np.asarray(depth))
            ...
            for log in image_logs:
                ...
                for normal, depth in zip(normals, depths):
                    normal = wandb.Image(normal, caption=validation_prompt)
                    formatted_images.append(normal)

                    depth = wandb.Image(depth, caption=validation_prompt)
...
def save_model_card(repo_id: str, image_logs=None, base_model=str, repo_folder=None):
    ...
    if image_logs is not None:
        ...
        for i, log in enumerate(image_logs):
            ...
            images = [validation_image] + normals + depths
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "dataset type, e.g., SUNRGBD, HYPERSIM, SCANNETPP"
        ),
    )
    parser.add_argument(
        "--dataset_name",
...
def make_train_dataset(args, tokenizer, accelerator):
    ...
    dataset = load_dataset(
        'json',
        data_files={
            'train': os.path.join(args.train_data_dir, 'train.jsonl'),
        },
        cache_dir=args.cache_dir,
    )
    ...
    def preprocess_train(examples):
        ...
        #     rgb = Image.open(os.path.join(args.train_data_dir, rgb_path)).convert('RGB')
        ...
        for semantic_path in examples['semantic']:
            semantic = Image.open(os.path.join(args.train_data_dir, semantic_path)).convert('RGB')
        ...
        for normal_path in examples['normal']:
            normal = Image.open(os.path.join(args.train_data_dir, normal_path)).convert('RGB')
        ...
        for depth_path in examples['depth']:
            depth = Image.open(os.path.join(args.train_data_dir, depth_path))
            assert args.dataset_type is not None
            if args.dataset_type == 'SUNRGBD':
                depth = np.asarray(depth, dtype=np.uint16)
                depth = parse_depth_func(depth, args.depth_type)
                # depth = torch.tensor(np.stack([depth, depth, depth]))
...
def collate_fn(examples):
    # rgb_pixel_values = torch.stack([example["rgb_pixel_values"] for example in examples])
    # rgb_pixel_values = rgb_pixel_values.to(memory_format=torch.contiguous_format).float()

    semantic_pixel_values = torch.stack([example["semantic_pixel_values"] for example in examples])
    semantic_pixel_values = semantic_pixel_values.to(memory_format=torch.contiguous_format).float()
    ...
    normal_depth_pixel_values = torch.cat([normal_pixel_values, depth_pixel_values], dim=1)
    normal_depth_pixel_values = normal_depth_pixel_values.to(memory_format=torch.contiguous_format).float()
    ...
    return {
        ...
        "normal_depth_pixel_values":    normal_depth_pixel_values,
        "input_ids":                    input_ids,
    }
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                latents = vae.encode(batch["normal_depth_pixel_values"].to(dtype=weight_dtype)).latent_dist.sample()
                ...
                controlnet_image = batch["semantic_pixel_values"].to(dtype=weight_dtype)
